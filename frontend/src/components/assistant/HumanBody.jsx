 ...
 *  - selectedParts (optional): array of body areas
 *  - onPartClick (optional): function(part)
 */
...
const HumanBody = ({
  ...
  const getHighlightClasses = () => {
    ...
    switch (riskLevel) {
      case "emergency":
        return {
          active: `fill-red-500 opacity-95 animate-pulse ${base} drop-shadow-[0_0_10px_rgba(248,113,113,0.8)]`,
          inactive: "fill-slate-300 opacity-55",
        };
      case "urgent":
        return {
          active: `fill-orange-400 opacity-95 ${base} drop-shadow-[0_0_10px_rgba(251,146,60,0.7)]`,
          inactive: "fill-slate-300 opacity-55",
        };
      case "self-care":
        return {
          active: `fill-emerald-400 opacity-95 ${base} drop-shadow-[0_0_10px_rgba(52,211,153,0.6)]`,
          inactive: "fill-slate-300 opacity-55",
        };
      case "routine":
      default:
        return {
          active: `fill-blue-400 opacity-95 ${base} drop-shadow-[0_0_10px_rgba(96,165,250,0.7)]`,
  ...
  const getPartClass = (part) => {
    ...
    if (highlightedPart === part) return active;
  ...
  return (
    <div className="w-full flex flex-col items-center justify-start">
      ...
      {/* Compact fixed-size body */}
      <div className="relative w-28 h-56 md:w-32 md:h-64 lg:w-36 lg:h-72 flex items-center justify-center">
        <svg
      ...
      {/* Caption */}
      <div className="mt-2 text-center text-[11px] text-slate-600 px-2">
        <div className="font-semibold text-slate-700">
          Highlighted area:{" "}
          <span className="text-slate-900">{prettyAreaLabel}</span>
        </div>
        <p className="mt-1 text-[10px] text-slate-500">
          Tap one or more areas where you feel discomfort.
        </p>
      </div>

      {/* Compact legend */}
      <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-[10px] text-slate-600 w-full max-w-xs">
